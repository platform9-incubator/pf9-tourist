      ...
      "Dashboard" shows this step-by-step wizard to guide you through getting started. </p>',
        ...
        setup: function (tour, options, view) {
            ...
            setTimeout(function () {
                ...
                $(".tourist-overlay").append("<div > <img id='auth-hosts' src='images/host-auth.gif'  style='display: block; margin-left: auto; margin-right:auto'> </div>");
                // tour.view.setTarget($('#auth-hosts'), self);
            }, short_wait)
            var xpos = Math.floor(window.innerWidth * .7);
            return {target: [xpos, 250]};
        },
        teardown: function (tour, options) {
            tour.view.hide()
        },
